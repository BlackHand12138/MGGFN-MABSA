...
class SpatialAttention(nn.Module):
    ...
    def forward(self, x):
        # x:(B,C,H,W)
        max_result, _ = torch.max(x, dim=1, keepdim=True)  # 通过最大池化压缩全局通道信息 返回通道维度上的: 最大值和对应的索引.
        avg_result = torch.mean(x, dim=1, keepdim=True)  # 通过平均池化压缩全局通道信息 返回通道维度上的: 平均值
        result = torch.cat([max_result, avg_result], 1)  # 在通道上拼接两个矩阵
        output = self.conv(result)  # 然后重新降维为1维
        return output


class CBAMBlock(nn.Module):

    def __init__(self, channel=256, reduction=16, kernel_size=7, HW=14*14):
        super().__init__()
    ...
    def forward(self, x):
        ...
        # 拼接,然后通过MLP建立相关性
        CS_x = torch.cat([Channel_x, Spatial_x], 1)
        CS_xx = self.MLP(CS_x)
        ...
        out1 = x * Channel_weight  # 将输入与通道注意力权重相乘
        out2 = x * Spatial_weight  # 将更新后的输入与空间注意力权重相乘
        return out1,out2
